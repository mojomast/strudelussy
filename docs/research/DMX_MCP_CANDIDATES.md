...
| DMXControl 3 | lighting console/app | Partly open ecosystem, main app Windows-only | Yes | gRPC/protobuf network interface. <https://github.com/DMXControl/DMXControl3-Network-Interface> | Weak for Linux-first dev | Good on Windows-centric installs | No public MCP support found | Low | Interesting network interface story | Windows-only main app, poor Linux fit | Reject for Strudelussy-first path |
